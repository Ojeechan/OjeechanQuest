# OjeechanQuest
Eclipseを用いたJavaプロジェクトを公開しています。
Eclipseで実行する場合以下の手順を行ってください。

  1. **Package Explorer**上で右クリック、**Import**を選択
  2. **Git**カテゴリより**Projects from Git**サブカテゴリを選択し**Next**
  3. **Clone URI**を選択し**Next**
  4. **URI**に https://github.com/Ojeechan/OjeechanQuest.git を入力し**Next**
  5. **Wizard for project import**まで進み、一番上の**Import existing Eclipse projects**を選択し**Next**
  6. **Finish**でインポート完了

--------------------------------------------------------------------------------------------------------------------------------

As derived from Java project with Eclipse, please refer to the following instructions:
  1. Right-click on **Package Explorer**, select **Import**
  2. Select **Git** > **Projects from Git**, click on **Next**
  3. Select **Clone URI**, click on **Next**
  4. Enter https://github.com/Ojeechan/OjeechanQuest.git into **URI**, click on **Next**
  5. Skip over next few steps until the wizard option, then select **Import existing Eclipse projects** and click on **Next**
  6. Click on **Finish**
